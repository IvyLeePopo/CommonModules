...
typedef void(*recvWebSocketMsg)(void* pUser, const char* pszData, unsigned int nSize);
typedef void(*recvWebSocketMsgConnectResult)(void* pUser, bool bConnected);

// 每一个websocket均基于线程基类，主要用于管理自己的消息
class WebSocketBase : public SingleTon<WebSocketBase>
{
...
public:
	...
	bool startSever1(/*const char* pszIp, const int nPort*/);
...
public:
	...
	// 设置回调函数---连接结果
	void setCallBackConnectResult(recvWebSocketMsgConnectResult pFunc, void* pUser);
	...
	// 消费发送消息函数
	void consumeSendMsg(struct lws *pWsi);

	// 接收到客户端的信息
	bool recvMsgByCallBack(const char* pszData, unsigned int nSize);
...
private:
	...
	list<pair<int, char*>> m_listMsg;							// 等待发送的消息队列
	...
	void* m_pUser1;												// 返回回调的对象
	recvWebSocketMsgConnectResult m_pRecvWebSocketFuncConnectResult;
...
#define WebSocketBaseServer WebSocketBase::getInstance()
